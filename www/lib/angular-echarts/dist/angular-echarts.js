(function () {
  ...
  function getLinkFunction($http, theme, util, type) {
    return function (scope, element, attrs) {
      ...
      var ndWrapper = element.find('div')[0], ndParent = element.parent()[0], parentWidth = ndParent.clientWidth,
        parentHeight = ndParent.clientHeight, width, height, chart;
      var chartEvent = {};
      ...
      function getOptions(data, config, type) {
        ...
        var options = {
          ...
          color: config.colors ? config.colors : ['#f1453d', '#fd9727', '#785549', '#b9d337',
            '#346823', '#1ebea5', '#6744b1', '#fed630', '#339aed', '#28e5fd', '#c4d839']
        };
        if (!config.showXAxis) {
  ...
  angular.module('angular-echarts.util', []).factory('util', function () {
    ...
    function getSeries(data, config, type) {
      ...
      angular.forEach(data, function (serie) {
        ...
        if (isPieChart(type)) {
          ...
          conf.center = config.center || [120, '50%'];
          conf.radius = config.radius || 90;
          // donut chart require special itemStyle
          if (type === 'donut') {
            conf.radius = config.radius || [46, 90];
            conf = angular.extend(conf, {
              itemStyle: {
                normal: {
                  label: config.label || {
                  ...
                  labelLine: config.labelLine || {show: false}
                },
                emphasis: {
                  label: config.emphasis || {
                    show: true,
                    formatter: '{b}\n{d}%',
                    position: 'center',
                    textStyle: {
    ...
    function getLegend(data, config, type) {
      ...
      if (isPieChart(type)) {
        ...
        legend.icon = 'circle';
        if (config.legend) {
          legend.orient = config.legend.orient || 'vertical';
          legend.x = config.legend.x || 'right';
          legend.y = config.legend.y || 'center';
          legend.align = config.legend.align || 'left';
          legend.icon = config.legend.icon || 'circle';
        }
      } else if (type === 'map') {
        legend = {};
      } else {
